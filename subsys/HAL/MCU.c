...
//     LOG_INFO("KEY start: %d", millis());
    ...
    if(readKeyVal()){
        ...
        tmos_start_task(halTaskID, HAL_SLEEP_EVENT, LOW_POWER_SLEEP_TIME_S);  //无键值变化30s睡眠
        tmos_start_task(halTaskID, HAL_SHUTDOWN_EVENT, LOW_POWER_SHUTDOWN_TIME_S);  //无键值变化30s睡眠
        switch(device_mode){
        case MODE_BLE:
...
//    {
//      adcsum += ADC_ExcutSingleConver();      // 连续采样20次
//    }
...
#define ADC_THRESHOLD (MIN_BATTERY_VOL*2048/1.05)  //低于2.5V睡眠
    ...
    tmos_start_task( halTaskID, HAL_ADC_EVENT, MS1_TO_SYSTEM_TIME(60*1000 ) );   //1min 检测一次
...
void HAL_Init()
{
  ...
  HAL_SleepInit();
  tmos_start_task(halTaskID, HAL_SLEEP_EVENT, LOW_POWER_SLEEP_TIME_S);  //无键值变化30s睡眠
  tmos_start_task(halTaskID, HAL_SHUTDOWN_EVENT, LOW_POWER_SHUTDOWN_TIME_S);  //无键值变化5min睡眠
#endif
