...
const uint8_t keytale_8b[] =                    //适配于丐17+4的键码表
{
//        R0   R1   R2   R3   R4   R5
...
{
    GPIOB_ModeCfg(
        GPIO_Pin_5| GPIO_Pin_4 | GPIO_Pin_14 | GPIO_Pin_15 ,  //这个顺序是从上到下
        GPIO_ModeOut_PP_5mA );//行
    GPIOB_ModeCfg(
        GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_2 | GPIO_Pin_3 | GPIO_Pin_12 | GPIO_Pin_13,GPIO_ModeIN_PU);//列
...
const uint32_t IOmap[] = {1<<5, 1<<14, 1<<4, 1<<15};  //这个顺序是从左到右
...
void keyScan(uint8_t *pbuf, uint8_t *key_num)//fristbuf 为这次扫描的按键情况   是配于丐17版本
{
    ...
    for( i = 0; i < 4; i++){ //4次列扫描，每次5个行按键，从左到右，从上到下
        GPIOB_ResetBits(IOmap[i]);//下拉列，读行
        __nop();__nop();  //由于上拉输入拉低需要一定的时间，所以必须延时一段时间再读IO
        {
            if (Key_S0 == 0) {
                Sataion = i * 7 + 1;  //firstbuf[KeyNum++]括号内KeyNum先调用后++,且是该键摁下才会++
    ...
    if(!GPIOB_ReadPortPin(GPIO_Pin_22))//将PB22(boot)复用为fn  单独扫描
    {
